...
async function post(path, body, opts = {}) {
  ...
  emitTiming(path, 0, 'send', 0, { body_bytes: jsonBody.length });
  ...
  if (!res.ok) {
    ...
    const totalElapsed = Date.now() - requestStart;
    emitTiming(path, 1, 'http-error', totalElapsed, { status: res.status });
  ...
  const totalElapsed = Date.now() - requestStart;
  emitTiming(path, 1, 'body-complete', totalElapsed);
  return json;
}
