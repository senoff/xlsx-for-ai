...
async function post(path, body, opts = {}) {
  ...
  // XLS-1768: an explicit `opts.bearer` (a device-flow OAuth access token) wins
  // over the stored api_key — used by register.js to call POST /api/v1/clients
  // with the OAuth bearer when the server requires it (flag ON), before any
  // durable api_key exists in config.
  if (opts.bearer) {
