...
function resolveGlobalBin() {
  ...
  try {
    // No interpolation of untrusted input — fixed argv to /bin/sh.
    const out = execFileSync('/bin/sh', ['-c', 'command -v xlsx-for-ai-mcp'], {
      encoding: 'utf8',
    }).trim();
    return out || null;
  } catch (_) {
    // `command -v` exits non-zero when the bin isn't on PATH.
    return null;
  }
}
...
function registerMcpServer({ mode = 'cli', log = (m) => process.stderr.write(m) } = {}) {
  ...
  if (parseError) {
    ...
    try { fs.copyFileSync(p, bak); } catch (_) { /* best effort */ }
