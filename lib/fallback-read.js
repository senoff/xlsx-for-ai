...
function safeCellText(cell) {
  try {
    ...
    // Match V8's exact null-deref TypeError shape — not any TypeError whose
    // message merely contains "null"/"undefined" (which would falsely
    // swallow legitimate errors like "argument must not be undefined").
    // Two alternations cover modern V8 ("…properties of null (reading 'x')")
    // and legacy V8 ("…property 'x' of null") — the latter shows up on
    // older Node runtimes some consumers may still pin to.
    const isNullishPropDeref = e instanceof TypeError && (
      /^Cannot read (?:property|properties) of (?:null|undefined)\b/.test(msg) ||
      /^Cannot read property '.*' of (?:null|undefined)\b/.test(msg)
    );
