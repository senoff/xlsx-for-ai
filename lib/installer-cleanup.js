...
function runCleanup({ confirm = false, log = (m) => process.stderr.write(m) } = {}) {
  ...
  for (const { p: t, kind } of targets) {
    ...
    // Fail-safe: without a RESOLVED current install to exclude, a shadowing
    // global could BE the authoritative install — list it, but never delete it.
    // Gate on currentReal (the realpath), not the raw `current` string: npm can
    // report a non-empty-but-wrong path (e.g. Windows lib/ layout), leaving
    // currentReal null while current is truthy — which would otherwise make the
    // real install deletable.
    const deletable = confirm && !(kind === 'shadow' && !currentReal);
    if (deletable) {
      // TOCTOU guard: the realpath checks above can be raced by swapping `t`
      // for a symlink between check and delete. Re-stat with lstat at the
      // delete site and refuse anything that isn't a real directory — a npm /
      // npx install dir always is, so a symlink here means the path changed
      // under us. This keeps the recursive delete on the inode we validated.
      let lst;
      try { lst = fs.lstatSync(t); } catch (_) { continue; }
      if (lst.isSymbolicLink() || !lst.isDirectory()) {
        log(`xlsx-for-ai cleanup: skipped ${t} — not a real directory at delete time.\n`);
        continue;
      }
      try {
