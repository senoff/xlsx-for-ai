...
function writeConfig(data) {
  ...
  // Defence in depth only, hence best-effort: a 0700 dir keeps others from traversing to the
  // file, but the FILE's own mode below is the load-bearing guard, so failing to tighten an
  // existing dir is not a security failure and must not take the write down with it.
  try { fs.chmodSync(dir, 0o700); } catch (_) { /* not load-bearing — see above */ }
